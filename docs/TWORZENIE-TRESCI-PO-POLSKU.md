...
Stan na 2026-07-16 (koniec sesji). Lokalizacja byla prowadzona jako wyczerpujacy przeglad calej bazy: diagnoza (szukanie angielskiej tresci) -> tlumaczenie -> test na zywo -> commit, przez kilkanascie kolejnych etapow.

**Zakonczone**: caly czasownikowy/notify-tell przeglad bazy (`$player`, `$room`, Generic Builder, Generic Wizard, Generic Programmer, Frand's Player Class, cala warstwa poczty, edytory, News, ANSI, Housekeeper, Registration DB, mniejsze "kieszenie" typu Site DB/Mr. Spell/Feature Warehouse/HTTP Server/MCP Package Registry i dziesiatki innych obiektow) oraz -- co okazalo sie osobnym, duzo wiekszym zadaniem -- pelny przeglad **wartosci wlasciwosci** (nie tylko wywolan `notify()`/`:tell()`) na wszystkich 13 obiektow bazy pomocy (`$code_utils:help_db_list()`). Ten drugi rodzaj przegladu ujawnil, ze glowna baza pomocy graczy (`#60`, ok. 90 ze 120 tematow, np. `help @password`, `help movement`, `help notes`) byla w duzej czesci nieprzetlumaczona -- niewidoczna dla metodologii szukania `notify()`/`:tell()`, bo tresc pomocy to wartosci wlasciwosci, nie wywolania czasownikow. Wniosek na przyszlosc: przy kazdym kolejnym przegladzie kompletnosci warto uruchomic obie metody, nie tylko szukanie wywolan `notify`/`:tell`.

**Przewodnik dla poczatkujacych -- ZROBIONE**: oryginalny angielski `https://lisdude.com/moo/toaststunt_newbie.txt` zostal przetlumaczony i zapisany jako [`docs/PRZEWODNIK-DLA-POCZATKUJACYCH.md`](PRZEWODNIK-DLA-POCZATKUJACYCH.md). Polska sekcja `docs/README.md` linkuje teraz do tego lokalnego pliku; angielska sekcja README nadal linkuje do oryginalu na lisdude.com, zgodnie z pierwotnym zamierzeniem (to byla zmiana wylacznie w sekcji polskiej).

**Wciaz otwarte**: zbudowanie przykladowego, przechodniego MOO (kilka/kilkanascie polaczonych lokacji, kilka przedmiotow, kilka NPC), zeby mozna bylo sie zalogowac, stworzyc postac i osobiscie sprawdzic dzialanie calosci w praktyce -- to byl warunek koncowy calego zadania lokalizacji, jeszcze nie podjety. Poza tym: dowolny kolejny przeglad kompletnosci (obu rodzajow, patrz wyzej) prawdopodobnie znajdzie jeszcze cos drobnego -- to normalne, nie oznacza to bledu w poprzednich etapach.
